// Header
// export const topHeaderBG = '#e23e57';
export const topHeaderBG = '#e84a5f';
export const topHeaderText = '#ffffff';
export const headerColor = '#2a363b';
export const headerTextColor = '#ffffff';
export const navBarBG = '#ccc';
export const navBarText = '#ffffff';
export const navBarTextDim = '#ccc';

// Main Body
export const mainColor = '#fefefe';
export const mainTextColor = '#2a363b';

// Lists
export const itemText = '#2a363b';
export const boardBackColor = '#ffffff';
export const borderColor = '#2a363b';
export const imageBorder = '#2a363b';
export const listBG = '#fff';
// export const cardTextColor = '#a3f7bf';


// // Header
// export const headerColor = '#303030';
// export const headerTextColor = 'white';

// // Main Body
// export const mainColor = 'white';
// export const mainTextColor = '#fff';

// // Lists
// export const itemText = '#333333';
// export const boardBackColor = 'white';
// export const borderColor = '#ccc';
// export const imageBorder = '#f5f5f5';

// // Standby colors
// export const topHeaderBG = 'tomatoe';
// export const topHeaderText = '#fff';
// export const navBarBG = '#ccc';
// export const navBarText = '#fff';
// export const navBarTextDim = '#ccc';
// export const listBG = '#fff';
